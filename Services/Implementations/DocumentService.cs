...
public sealed class DocumentService : IDocumentService
{
    ...
    public async Task<ApiResponse<DocumentResponse>> CreateAsync(CreateDocumentRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var cashboxExists = await _context.CheckDefs.AsNoTracking()
            .AnyAsync(x => x.Id == request.IdSandogh && x.Type == request.IdSandoghType, cancellationToken);
        if (!cashboxExists)
            ...
            if (!item.IsIncoming && request.CheckStock)
            {
                ...
                if (!stock.IsAvailable)
                {
                    stockWarnings.Add(new
                    {
                        ...
                        stock.KalaId, stock.IdAnbar, stock.IdSal, stock.Requested, stock.Available, stock.IsAvailable
                    });
                }
            }
        }
        ...
        var sanad = new Sanad
        {
            ...
            MabCheck = 0, MabBed = 0, IdMasool = request.IdMasool, IdTaiid = null, Des = request.Des, IDEijad = null,
            IdDoreh = null, CountGhest = 0, DarsadGhest = 0, Maliat1 = 0, Maliat1Darsad = 0, Maliat1Sel = false,
            Maliat2 = 0, Maliat2Darsad = 0, Maliat2Sel = false, MabHarGhest = 0, MabKolAghsat = 0, GhestSel = false,
            KarmozdFrosh = 0, TarafName2 = null, Sharh = request.Sharh, Takhfif2 = 0, IsTasvieh = false, TasviehID = 0,
            Disable = false, IDSanadEx = 0, IDSanadEx2 = 0, IDSanadEx3 = 0, ShowInSanad = true, ShowInFaktor = true,
            TasviehDate = request.SabtDate, IsTasviehDate = false, IDSandogh = request.IdSandogh, IDSandoghType = request.IdSandoghType,
            MabKart = 0, MabFish = 0, IDKart = 0, IDTypeKart = 0, IsFinal = true, IDFroshMabType = 0,
            SanadTime = DateTime.Now.ToString("HH:mm:ss"), TakhfifKala1 = false, TakhfifKala2 = false, TakhfifKala3 = false,
            IsMaliat1Darsad = false, IsMaliat1Kala = false, IsMaliat2Darsad = false, IsMaliat2Kala = false, IsPorsant = false,
            IsPorsantMabKol = false, IsPorsantMabKala = false, HazFaktor = 0, IDHazFaktor = 0, HazFaktor2 = 0, IDHazFaktor2 = 0,
            TakhfifDarsad = 0, TakhfifOnvan = null, MabEzaf = 0, MabEzafDarsad = 0, MabEzafOnvan = null, SefareshID = null,
            IsSavedFinal = true, IDSanad = 0, Takhfif3 = 0, TakhfifKala = 0, IDFish = 0, IDFoodMahal = 0, Tel = null, Add = null,
            CodeMeli = null, Miz = null, GpsLat = 0, GpsLong = 0, TasvieType = 0, TasvieCheck = 0, IDAnbar2 = request.IdAnbar,
            IDTaraf2 = request.IdTaraf, HMarketID = 0, IDRef = "", SanadTypeRef = 0, IDRefRecive = "", FroshArzesh = null,
            TakhfifKalaArzesh = null, HMaliat1 = null, HMaliat2 = null, TejaratCode = null, StateMaliat = 0, SabtDateOrg = request.SabtDate,
        ...
        foreach (var item in request.Items)
        {
            ...
            details.Add(new SanadDetail
            {
                ...
                HazKala2 = null, HazKala3 = null, SumTakhfifKala = 0, HazKala1 = null, HazKalaGift1 = null, HazKalaGift2 = null,
                HazKalaGift3 = null, IdAttribValuesStock = "", TakhfifD2 = null, TakhfifD3 = null, TakhfifMab1 = null,
                TakhfifMab2 = null, MaliatD1 = null, MaliatD2 = null, TasviehRoz = null, MaliatMab1 = null, MaliatMab2 = null,
                SumMabTakh = null, SumMabMaliat = null, MabFroshByTakh = null, Bed2 = item.IsIncoming ? (double)item.Quantity : 0,
                Bes2 = item.IsIncoming ? 0 : (double)item.Quantity, BedMab2 = item.IsIncoming ? unitPrice : 0,
                BesMab2 = item.IsIncoming ? 0 : unitPrice, MabEzafatMoaf = null
            });
        }
        ...
        if (stockWarnings.Count > 0)
            return ApiResponse<DocumentResponse>.SuccessWithWarningResult(response, stockWarnings, "سند ثبت شد، اما موجودی یک یا چند کالا کافی نبود.", "STOCK_WARNING");
    ...
    public async Task<ApiResponse<IReadOnlyList<DocumentResponse>>> GetHistoryAsync(int idSal, int sanadType = 12, int page = 1, int pageSize = 30, CancellationToken cancellationToken = default)
    {
        ...
        var sanads = await _context.Sanads.AsNoTracking()
            ...
            .OrderByDescending(x => x.IdFaktor).ThenByDescending(x => x.Id)
            .Skip((page - 1) * pageSize).Take(pageSize).ToListAsync(cancellationToken);

        if (sanads.Count == 0)
            return ApiResponse<IReadOnlyList<DocumentResponse>>.SuccessResult(Array.Empty<DocumentResponse>(), "تاریخچه سندی ندارد.");
        ...
        return ApiResponse<IReadOnlyList<DocumentResponse>>.SuccessResult(result, "تاریخچه اسناد با موفقیت دریافت شد.");
    }
    ...
    {
        var max = await _context.Sanads.AsNoTracking().Where(x => x.IdSal == idSal).Select(x => (int?)x.IdFaktor).MaxAsync(cancellationToken) ?? 0;
        return max + 1;
    }
    ...
    private static DocumentResponse Map(Sanad sanad, IReadOnlyCollection<SanadDetail> details, string? tarafName = null)
    {
        ...
        {
            IdSal = sanad.IdSal, Id = sanad.Id, SanadType = sanad.SanadType, IdAnbar = sanad.IdAnbar,
            IdTaraf = sanad.IdTaraf, IdTarafType = sanad.IdTarafType, IdFaktor = sanad.IdFaktor, SabtDate = sanad.SabtDate,
            TotalAmount = sanad.MabKol, IsFinal = sanad.IsFinal, Description = sanad.Des, TarafName = tarafName,
