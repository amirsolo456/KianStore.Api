...
- `POST /api/documents/purchase?sanadType=` — purchase document; all lines are forced to incoming (`Bed/B ed2`), so a successful purchase increases stock through the existing `SanadDetail` stock model
...
For IIS/HTTPS, replace the base URL with the final internal/external API URL.
