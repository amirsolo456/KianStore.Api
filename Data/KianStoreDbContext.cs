...
public class KianStoreDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<DiscountCode>(entity => { entity.HasKey(x => x.Id); entity.HasIndex(x => x.Code).IsUnique(); entity.Property(x => x.Code).HasMaxLength(50).IsRequired(); entity.Property(x => x.Title).HasMaxLength(200); entity.Property(x => x.Value).HasPrecision(18, 3); entity.Property(x => x.MaxDiscountAmount).HasPrecision(18, 3); entity.Property(x => x.StartDate).HasColumnType("datetime2"); entity.Property(x => x.EndDate).HasColumnType("datetime2"); entity.Property(x => x.Description).HasMaxLength(1000); entity.Property(x => x.CreatedAt).HasColumnType("datetime2"); entity.HasOne<Takhfif>().WithMany().HasForeignKey(x => x.TakhfifId).OnDelete(DeleteBehavior.Restrict); });
